    ...
    <div class="flex justify-center">
        <div class="w.full md:w-8/12 lg:w-6/12 flex flex-col item-center md:flex-row">
            ...
            <form method="POST" action="{{route('users.follow',$user)}}">
                @csrf
                <input value="seguir" class="bg-blue-600 text-white uppercase rounded-lg px-3 py-1 text-xs font-bold cursor-pointer" type="submit">
            </form>
            ...
            <form method="POST" action="{{route('users.unfollow',$user)}}">  
                @csrf
                @method('DELETE')
                <input value="Dejar de seguir" class="bg-red-600 text-white uppercase rounded-lg px-3 py-1 text-xs font-bold cursor-pointer" type="submit">
            </form>
            @endif
            @endif
            @endauth
