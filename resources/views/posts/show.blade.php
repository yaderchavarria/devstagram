...
@section('contenido');
    <div class="container mx-auto md:flex p-14">
        <div class="md:w-1/2">
           ...
           @if ($post->user_id === auth()->user()->id)
            ...
            <form method="POST" action="{{route('posts.destroy',$post)}}">
                @method('DELETE')
                @csrf
                <input class="bg-red-500 hover:bg-red-600 p-2 rounded text-white font-bold mt-4 cursor-pointer" type="submit" value="Eliminar Publicación">
            </form>
            @endif
            @endauth
        </div>
